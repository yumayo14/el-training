...
- ワイヤーの完成
  - 実際のユーザーを想定した、より緻密なプロトタイプを作成する(must)
  - 色、大まかな配置、使用するフォントサイズなどを決定する(must)
  
- 画面の改修
  - Issueの一覧表示画面を改修(must)
  - Issueの詳細ページを改修（ステップをクリックした際のhtmlはここでは記入しない）(must)
  - Issueの詳細ページでステップをクリックすると表示される、タスクのリスト画面を作成(must)

- Issue  
  - Issueを投稿できるようにする(must)
  - Issueを編集できるようにする(must)
  - Issueを削除できるようにする(must)
  - Issueの名前でフィルタリング検索ができるようにする
  - Issueの状態でフィルタリング検索ができるようにする(must)
  - Issueの詳細ページへ遷移できるようにする

- Step
  - ユーザーがissueを投稿するたび、ステップが3つ作成されるようにする(must)
  - 各ステップを編集できるようにする(must)
...
- Task
  - 各ステップに紐づくタスクを作成できる(must)
  - 各ステップに紐づくタスクを編集できる(must)
    ( クリックすると表示部分が編集できるようになる形で実装したい(Jsでゴニョゴニョする) )
  - 各ステップに紐づくタスクを削除できる(must)
  - 各ステップに紐づいているステップの順番を並び替えられる(must)
