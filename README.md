          ...
          - なぜ中断するのか
            1. 時間がない
                ...
                - ただ「問題」と「想定する挙動」だけを管理している場合
                  - 時間ができた時に思い出して、その後再開する手助けにはなるかも（google keepでよくない？）
            2. 飽きる
                - 実装していくと意外と面倒で、やる気が損なわれていく
                - やり始めると、あれもこれもとなってしまい、結局どこで終わるかが不明確になってしまう
            3. 解決しようとした問題が大きすぎた
                - 明らかにプログラムだけでは解決不可能な問題だったことがある

        - 中断を防ぐ仕組み
          - 2: 「飽きる」を防止する方法
          - どこまでやれば終わりかが分かるようにする
            - 具体的な実装案
              - ドキュメントにプログラム完成のために必要なタスクを登録できるようにする仕組み
              - 完成のために必要なタスクを一覧表示できるようにする仕組み
              - 登録した問題の解決度が何%ぐらいのところまできているかが表示されるようにする
          - 3: 「解決しようとした問題が大きすぎた場合」を防止する方法
            - アプローチ可能なレベルにまで問題を分解する
